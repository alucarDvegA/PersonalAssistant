...
## 設定方法
```/setting```とだけ入力すると設定モードに切り替わります。その状態で以下のコマンドで各種設定を変更できます。
...
ex)change_persona:女の子として、恋愛的に好きな相手に対する返答をため口で２０文字でしてください。
...
speaker_id:番号
キャラクターの声を指定した番号に変更します。指定できる声の一覧はリプライかgithubのREADMEに書いてます。最新の声の一覧はVOICEVOXのアプリを起動した状態で
